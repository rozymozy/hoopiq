...
(function() {
  ...
  function injectNav() {
    ...
    // Compute auth link BEFORE the template literal — avoids nested backticks entirely
    let authLink = '';
    try {
      const sess = JSON.parse(localStorage.getItem('courtside_session') || 'null');
      if (sess && sess.access_token) {
        authLink = '<a class="hn-link ' + (current === 'dashboard.html' ? 'active' : '') + '" href="dashboard.html">Dashboard</a>';
      } else {
        authLink = '<a class="hn-link ' + (current === 'auth.html' ? 'active' : '') + '" href="auth.html">Sign in</a>';
      }
    } catch(e) {
      authLink = '<a class="hn-link" href="auth.html">Sign in</a>';
    }
